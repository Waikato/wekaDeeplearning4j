...
/**
 * Copy paste version of {@link org.deeplearning4j.iterator.CnnSentenceDataSetIterator} due to its
 * restriction to nominal classes. See also:
 * https://gitter.im/deeplearning4j/deeplearning4j?at=5a2f3ab4c65707ba2b3edfae
 *
 * <p>Extended the label INDArray generation for regression tasks by checking numClasses > 1.
 *
 * <p>A DataSetIterator that provides data for training a CNN sentence classification models (though
 * can of course be used for general documents, not just sentences. The iterator handles conversion
 * of sentences to training data for CNNs, where each word is encoded using the word vector from the
 * specified WordVectors (i.e., word2vec etc) model.<br>
 * Labels are encoded using a one-hot representation and are 2d - i.e., are intended to be used with
 * a model that utilizes global pooling.<br>
 *
 * <p>Specifically:<br>
 * - Features have shape [minibatchSize, 1, maxSentenceLength, wordVectorSize] OR [minibatchSize, 1,
 * wordVectorSize, maxSentenceLength] depending on the configuration (for sentencesAlongHeight =
 * true/false respectively)<br>
 * - Labels are a 2d array with shape [minibatchSize, numLabels].<br>
 * Sentences and labels are provided by a {@link LabeledSentenceProvider} - different
 * implementations of this provide different ways of loading sentences/documents with labels - for
 * example, from files, etc.
 *
 * <p><b>Note</b>: With regard to labels to class index assignment, they are sorted alphabetically.
 * To get the assigment/mapping, use {@link #getLabels()} or {@link #getLabelClassMap()}
 *
 * @author Alex Black
 */
@AllArgsConstructor
public class CnnSentenceDataSetIterator implements DataSetIterator {
  ...
  private CnnSentenceDataSetIterator(CnnSentenceDataSetIterator.Builder builder) {
  ...
  private INDArray getVector(String word) {
    ...
    if (unknownWordHandling == UnknownWordHandling.UseUnknownVector
        && word
            == UNKNOWN_WORD_SENTINEL) { // Yes, this *should* be using == for the sentinel String
                                        // here
      vector = unknown;
    } else {
      if (useNormalizedWordVectors) {
  ...
  public List<String> getLabels() {
    // We don't want to just return the list from the LabelledSentenceProvider, as we sorted them
    // earlier to do the
...
//      // we need to pre-load some tokens. Otherwise, sentenceProvider could have 1 (invalid)
//      // sentence
...
//    return preLoadedTokens != null;
  ...
  public DataSet next(int num) {
    ...
    int minLength =
        Integer
            .MAX_VALUE; // Track to we know if we can skip mask creation for "all same length" case
...
//      featuresMask = Nd4j.create(currMinibatchSize, 1, maxLength);
//
//      for (int i = 0; i < currMinibatchSize; i++) {
...
//          featuresMask
//              .get(NDArrayIndex.point(i), NDArrayIndex.interval(0, sentenceLength))
//              .assign(1.0);
//        }
//      }
//    }
  ...
  public static class Builder {
    ...
    private CnnSentenceDataSetIterator.UnknownWordHandling unknownWordHandling =
    ...
    /** Set stopwords */
    public CnnSentenceDataSetIterator.Builder stopwords(AbstractStopwords sw) {
      this.stopwords = sw;
      return this;
    }
    ...
    /** Specify how the (labelled) sentences / documents should be provided */
    public CnnSentenceDataSetIterator.Builder sentenceProvider(
        LabelAwareDocumentIterator iterator, @NonNull List<String> labels) {
    ...
    /** Specify how the (labelled) sentences / documents should be provided */
    public CnnSentenceDataSetIterator.Builder sentenceProvider(
        LabelAwareSentenceIterator iterator, @NonNull List<String> labels) {
    ...
    /**
     * Specify how unknown words (those that don't have a word vector in the provided WordVectors
     * instance) should be handled. Default: remove/ignore unknown words.
     */
    public CnnSentenceDataSetIterator.Builder unknownWordHandling(
    ...
    public CnnSentenceDataSetIterator.Builder useNormalizedWordVectors(
    ...
    /**
     * Maximum sentence/document length. If sentences exceed this, they will be truncated to this
     * length by taking the first 'maxSentenceLength' known words.
     */
    ...
    /**
     * If true (default): output features data with shape [minibatchSize, 1, maxSentenceLength,
     * wordVectorSize]<br>
    ...
    public CnnSentenceDataSetIterator.Builder dataSetPreProcessor(
